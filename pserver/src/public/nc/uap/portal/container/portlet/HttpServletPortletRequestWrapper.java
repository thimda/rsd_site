 ...
 * @version $Id: HttpServletPortletRequestWrapper.java 949141 2010-05-28 10:24:49Z ate $
 */
        ...
        else
        {
            if (dispatchDetection == DispatchDetection.EVALUATE)
            {
                if (isRequestWrapperStackChanged())
                {
                    ...
                    {
                    	LfwLogger.debug("DispatchDetection: changing from EVALUATE to CHECK_WEBCONTAINER_REQUEST");
                    }
            ...
            if (!dispatchDetected)
            {
                ...
                if (dispatchDetected && dispatchDetection == DispatchDetection.EVALUATE)
                {
                    if (LfwLogger.isDebugEnabled())
                    {
                    	LfwLogger.debug("DispatchDetection: changing from EVALUATE to CHECK_STATE");
                    }
